...
%% ---------------------------------------------------------------------------
 ...
 subgraph UIScreens["**UI Screens**"]
    ...
    %% PreferencesScreen now contains tabs
...
| **File Storage**       | Firebase Storage (Secure Signed URLs)                                                                                  |
...
*   **Firebase Storage Security & Access Model:**
    ...
    * Haikuverse employs two distinct, parallel security models for its Storage assets, a key architectural decision based on the operation's context and trust requirements:
        * **Client-Side Access (via Firebase SDK):** This model is used for standard haiku images generated in the `StarsScreen`. The client uploads the file directly, receives a standard Firebase download URL (with a token), and access is governed by **Storage Security Rules** that evaluate the user's authentication state (`request.auth`). This is a simpler, user-centric model for low-trust operations.
        * **Server-Side Access (via Cloud Functions):** This model is used for profile images (which require sanitization) and audio files (which require Text-to-Speech processing). The client calls a Cloud Function, which runs as a trusted service account. Access is granted by generating a **Signed URL**. The validity of this URL depends entirely on the **Google Cloud IAM permissions** (`Storage Object Viewer`, `Service Account Token Creator`) granted to the function's service account. This is a more complex but necessary model for operations that must be performed in a secure, server-side environment.
