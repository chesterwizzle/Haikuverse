...
*   **Firebase Storage Security & Access Model:**
    ...
    *   **Access via Token-Secured URLs:** Publicly visible assets are secured via unguessable download tokens embedded in their URLs. While the Storage Security Rule for reads may appear permissive (`allow read: if request.auth != null`), this rule primarily serves to gate access to authenticated users. The actual security relies on the principle that these long-lived, tokenized URLs are computationally impossible to guess. Access is granted only to a client that possesses the complete URL, which is delivered securely from Firestore or a Cloud Function to an authenticated session. This architecture is designed to be seamlessly upgraded to a stricter model where rules will additionally validate Firebase App Check tokens for every read request.
