...
*   **`vertex_ai_service.dart` (`VertexAIService`):**
    *   **Purpose:** Encapsulates direct client-side calls to the Firebase Vertex AI SDK for image generation. Refactored to support dynamic model injection, allowing the application to seamlessly switch between Imagen 3 (default/fallback) and Imagen 4 (high-fidelity) based on feature requirements.
    *   **Methods:**
...
### Visual Cohesion in the Knowledge Graph (The Imagen 4 Upgrade)
A deliberate design constraint within the Haikuverse is that users cannot directly prompt the visual generation of Constellation Fables. This restriction exists to enforce a unified visual language across the semantic Knowledge Graph. Because users navigate from constellation to constellation based on thematic connections, jarring shifts in aesthetic style would break the immersion of the "universe."

To compensate for this lack of direct user control, the backend utilizes **Imagen 4** specifically for Fable generation. This higher-fidelity model acts as a "visual conductor," capable of interpreting the AI-generated fable text with a nuance and consistency that ensures every node in the knowledge graph feels like it belongs to the same cohesive reality. While users define the meaning (via the fable), the system strictly curates the aesthetic to ensure the Haikuverse remains a seamless, interconnected realm.
