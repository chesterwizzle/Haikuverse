...
* **Co-Create Techno-Poetic Art:** Guide powerful AI partners (**Gemini 2.5**, **Imagen 4**, and **Text-to-Speech**) to transform your creative spark into multi-sensory experiences. Move beyond simple generation to augment your artistry, weaving evocative haiku, stunning visuals, and dynamic audio narrations into a unified whole.
...
* **Explore a Living Universe of Poetry:** Navigate a unique, procedurally generated vector-art cosmos where every star is a poem waiting to be discovered. This is not a static gallery; it's an emergent universe that responds to you in real-time. Watch as stars visually evolve with community appreciation, gaining vibrancy and unique coronas that reflect their shared impact.
...
* **Connected Public Ecosystem & Premium Manufacturing:** Seamlessly integrated with the **Haikuverse Public Gallery**, a high-performance Next.js companion app that acts as an SEO-optimized "front door" to the ecosystem. This dual-purpose platform functions as a robust acquisition funnel by breaking down "walled gardens" to make content globally discoverable, while simultaneously serving as a secure **Print-on-Demand E-Commerce Engine**. Its sophisticated fulfillment pipeline supports dual-layer assets to produce premium **Metallic Foil** prints, allowing world-builders and creators to mint their digital lore into tangible, shimmering artifacts suitable for tabletop props or distinct keepsakes.

* **Master the Art of Prompting:** Demystify the creative process with Prompt Recovery. View and copy the original "Master Prompt" from any published star. This feature is designed to turn the Haikuverse into a learning lab, allowing poets to study successful techniques, iterate on their ideas, and share their exact recipes with the **r/HaikuverseRealms** community.
...
| **Serverless Backend** | Google Cloud Functions (Node.js 22)                                                                                       |
| **Generative AI**      | **Vertex AI**: Gemini (Text), Imagen 3 & 4 (Images), Text-to-Speech (Chirp3 HD Voices), Embedding API                  |
...
*   **`home_screen.dart` - Haiku & Star Name Generation:**
    ...
    *   **Key UI:** A `TextField` for prompts that is paired with an instructional "Welcome, Poet!" message on initial load that educates users on the "Master Prompt" concept to improve their first-time experience. The screen also features a "clear" button, a "Generate Haiku" `ElevatedButton`, a Star Naming Toolkit, a `FavoriteButton`, and a `StarProgressTracker` checklist.
    *   **Logic:**
...
*   **`favorites_screen.dart` - Favorites Display, Removal, and Voice Pairing Interface:**
    ...
    *   **Key UI:** `ListView.builder` rendering `ListTiles` for each haiku in reverse chronological order (newest first) to improve workflow. Each tile shows a numbered index, haiku text, a star name, an `IconButton` to trigger the `AudioToolkitModal`, and a delete `IconButton`.
    *   **Logic:**
...
*   **`stars_screen.dart` - AI Image Pairing and Favorite Exploration Interface:**
    * **Purpose:** Enables users to generate and associate AI-powered images (Imagen 4 via `VertexAIService`) with their favorite haikus and manage the slideshow order.
...
*   **`constellations_screen.dart` - Haikuverse Community Hub & Constellation Management:**
    ...
    *   **Logic:** Acts as a container for `PublishTab` (featuring a `Prompt Recovery` modal to view and copy original inputs), `ManageTab`, and `ExploreTab`, passing down necessary services and data streams.
...
*   **`sharing_tab.dart`  - Haiku Card Sharing & Outreach:**
    ...
    *   **Logic:**
        ...
        *   **Platform-Aware Sharing:** The sharing logic is now fully platform-aware. On native mobile and mobile web browsers (where the Web Share API is supported), it uses the `share_plus` package to invoke the native OS share sheet. On desktop web browsers, it provides a user-friendly fallback, triggering a direct download of the haiku card image. The dialog's action button dynamically updates its text to "Share" or "Download" to match the platform's capability.
...
*   **`haikuverse_navigation_screen.dart` - Immersive 2D Constellation Visualization:**
    ...
    *   **Real-Time State Synchronization:** The Haikuverse Navigation screen now subscribes to a PublishedStar stream to receive live updates. It uses a "Stale-but-Disabled" UI model to prioritize a stable user experience:
        *   **Like Updates:** Changes to a star's likeCount are updated on the local `StarNode` and the painter re-renders the star's visual rank instantly, without a full layout recalculation.
        *   **Unpublish Events:** If a star is unpublished, its `isPublished` flag is set to `false`. The painter then renders the star and its connections in a "faded" or "disabled" state, preserving the graph's structural integrity. Tapping a disabled star is intercepted and shows an informative snackbar.

Stable Layout: To prevent disorienting UX, new stars added to the constellation are intentionally ignored by the live stream. The new graph layout is only calculated when the user leaves and re-enters the screen.        
...
*   **`constellation_customization_screen.dart`:** Allows constellation owners to generate and save AI-created fables and images, and manages the knowledge graph update workflow. Implements a "Graceful Fallback" resilience pattern for Fable imagery, prioritizing Imagen 4 for high-fidelity results while automatically reverting to Imagen 3 on failure to guarantee user success without consuming extra quota.
...
*   **Platform Detection & Download Helpers (`platform_*.dart`):** To solve a critical cross-platform build failure, all web-only code (`package:web`, `dart:js_interop`) was isolated using a conditional import/export pattern. This provides platform-aware functions like `isMobileBrowser()` (to detect mobile vs. desktop browsers) and `downloadImage()` (for the desktop download fallback) while ensuring the native Android build never attempts to compile web-specific libraries.
...
*   **`storage_service.dart` (`StorageService`):**
    ...
    * **Methods:**
        ...
        * `deleteFileByUrl(String? fileUrl)`: DDeletes any file (image or audio) from Storage given its download URL. Robustly handles all formats: standard `firebasestorage.googleapis.com` download URLs, legacy `gs://` URIs, and backend-generated `storage.googleapis.com` Signed URLs, ensuring no orphan files are left behind regardless of how they were created.
...
*   **`vertex_ai_service.dart` (`VertexAIService`):**
    *   **Purpose:** Encapsulates direct client-side calls to the Firebase Vertex AI SDK for image generation. Refactored to implement a **Resilient High-Fidelity** strategy. It prioritizes Imagen 4 for superior lighting and semantic understanding but includes intelligent `try-catch` logic to automatically fallback to Imagen 3 in the event of quota limits or service availability issues, ensuring user creativity is never blocked.
    *   **Methods:**
        *   `generateImages(String prompt, {String? modelName})`: Initializes the Imagen model and calls `model.generateImages()`. Accepts an optional model constant to override defaults or trigger fallbacks.
...
*   **Haikuverse Publishing, Discovery & Lifecycle:**
    ...
    *   `deletePublishedStar` (HTTP, v2): Removes a published haiku. Atomically updates Firestore (decrementing counts, handling ownership transfer, deleting empty constellations) and removes the corresponding datapoint from the `Vector Search` index. Crucially, it enforces a "Clean Slate" protocol by recursively purging all associated subcollections (`user_likes`, `comments`, `pending_comments`, `blocked_users`) to prevent orphan data from corrupting future republishing attempts. Finally, it performs a fault-tolerant sync to reset the `likeCount` and clear the `constellationId` on the user's private favorite document.
...
*   **Monetization & Quota Management:**
    ...
    * `handlePlayStoreNotification` (Pub/Sub Trigger, v2): An event-driven listener for Google Play's play-billing topic. It processes `RENEWED`, `EXPIRED`, and `CANCELED` notifications in real-time to manage entitlements.
    * `resetManualAccountQuotas` (Scheduler Trigger, v2): A daily scheduled function that grants quota resets only for non-Play-Store, manual `hasForeverAccess` accounts.
...
*   **E-Commerce Fulfillment & Privacy (Shared Backend Logic):**
    * `onOrderPaid` (Firestore Trigger, v2): The sophisticated fulfillment orchestrator. It listens for paid orders and manages the secure hand-off to the `Gelato API (v4)`. It dynamically constructs manufacturing payloads, handling standard prints as well as complex Metallic Foil orders by detecting and injecting specific foil mask layers. It utilizes distinct Signed URL strategies (V4 for manufacturing, V2 for email persistence) to securely bypass App Check firewalls during the server-to-server transfer.
...
*   **Image Generation & Association Workflow (User Action -> `StarsScreen` -> Vertex AI SDK -> Storage -> `main.dart` -> Firestore -> Stream Update):**
    * In `StarsScreen`, "Generate Image" first triggers a call to the `consumeImageQuota` Cloud Function via `FirestoreService`. This backend function atomically verifies and decrements the user's available image quota in a Firestore Transaction. Only upon success does the client proceed to call `VertexAIService`. This service attempts generation using the Imagen 4 (High Fidelity) model first. If this fails (e.g., due to rate limiting), it transparently catches the error and retries using the Imagen 3 model, ensuring a result is delivered. `StorageService` then uploads the resulting image bytes to Storage. The returned URL is tracked by `CurrentImageProvider` as unsaved. On save, `_associateImageUrlToHaiku` in `_AppShellState` calls `FirestoreService.updateFavorite` to persist the new URL to the `Haiku` document in Firestore and notifies `CurrentImageProvider` to mark the image as saved. Additionally, users can launch the `ReorderImagesScreen` to visually drag and drop their three saved images into a new order, which is then persisted back to the `Haiku` document in Firestore via the `_saveReorderedImages` callback in `_AppShellState`.
...
*   **Haiku Publishing Workflow (`PublishTab` -> `FirestoreService` -> Cloud Functions -> Vector Search / Firestore):**
    *   The user selects a haiku in `PublishTab`. The tab fetches recommendations via the `generateConstellationRecommendations` Cloud Function, which queries the "published stars" Vector Search index. Confirming publication triggers the `publishStar` Cloud Function, which atomically updates Firestore, embeds the haiku, and upserts it to the Vector Search index. The client interface immediately reflects this change via a **reactive stream listener**, ensuring the "Publish" button transforms into a "Manage" interface instantly, even before the transaction is fully settled on the server.
...
* **Node.js and npm:** Required for Firebase Functions. **Node.js version 22 is required by the production environment.** Using Node Version Manager (`nvm`) is highly recommended. 
...
* **Enabled Google Cloud APIs:** In your Google Cloud project, ensure the following APIs are enabled:
    ...
    * Cloud Functions API.
    * Cloud Scheduler API (for `zeitgeistEngineScheduled` and `resetManualAccountQuotas`).
    * Google Play Developer API (for in-app purchase validation).
    * Pub/Sub API (for Google Play RTDN via the play-billing topic).
...
Haikuverse App is foundationally architectedupon a **serverless-first** client-server model, ensuring robust security, operational efficiency, and inherent scalability. The cross-platform Flutter client delivers a rich user experience, handling UI rendering, local logic, and sophisticated client-side prompt engineering. Computationally intensive and security-critical operations are delegated to a secure backend using Google Cloud Functions. This serverless approach eliminates infrastructure management and is complemented by an **event-driven** model where appropriate. For instance, the `updateLikeCount` and `syncFavoriteMediaToPublishedStar` functions are not called directly by the client but are Firestore triggers that react to database events. This decouples complex background tasks from the user-facing request cycle, creating a resilient system where data aggregation and synchronization happen automatically and reliably. Computationally intensive, security-critical, and transactional monetization operations are delegated to a secure backend using Google Cloud Functions, which leverages the trusted, globally distributed infrastructure of the **Google Play Store** for all in-app purchases. This serverless approach eliminates infrastructure management
...
Haikuverse App employs a pragmatic, multi-faceted hybrid state management strategy to optimize for distinct data characteristics and user experience needs. This combines Cloud Firestore's **real-time streams** for dynamic, multi-device data synchronization (e.g., favorite haikus via `_AppShellState` and `FirestoreService.getFavoritesStream`), `SharedPreferences` for efficient **local caching**, and Flutter's `provider` package for orchestrating **global state**. This strategy is further refined by dedicated providers like `NavigationProvider`, which decouples global UI navigation from content-specific state to prevent unintended side effects and improve modularity. Local `StatefulWidget` state (via `setState`) manages transient, UI-centric logic within individual components. This is exemplified in the `AudioToolkitModal` for immediate feedback, and in the `HomeScreen`, where an "Immediate Cache Fallback" pattern allows the `StarProgressTracker` to reset instantly upon re-favoriting, bridging the gap between user action and asynchronous Firestore consistency. Use of mounted checks and careful use of `context.read/watch` prevent common pitfalls with asynchronous operations, while **cache-busting** (appending unique query parameters to URLs) ensures freshness of externally fetched media like audio. This strategy is further refined by reactive stream listeners in critical UI paths, such as the `PublishTab`. Instead of relying on one-time fetching, the tab subscribes to a real-time stream of the specific star's publication status. This ensures that even if the local cache returns stale data on a cold start, the UI automatically self-corrects the moment the cloud sync completes, eliminating "ghost data" without user intervention.

This hybrid model's strengths are best demonstrated in the `HaikuverseNavigationScreen`, which serves as a capstone example of the entire philosophy. The screen uses a one-time `Future` to fetch data and algorithmically calculate its static procedural layout, **locking in the user's spatial context**. Simultaneously, it subscribes to a Firestore `Stream` to receive dynamic updates for star `likeCount` and `isPublished` status. This intentional separation of static and dynamic state enables the "Stale-but-Disabled" UI model: star ranks update in real-time, and unpublished stars gracefully fade out, all while intentionally ignoring newly added stars. This approach prevents chaotic, disorienting layout redraws and explicitly prioritizes the user's stable, focused experience over disruptive, albeit "live," data changes.
...
A core architectural strategy for maintaining modularity is a firm commitment to **platform-aware design**. The application acknowledges that a true cross-platform experience is not about writing one piece of code that runs everywhere, but about writing code that intelligently adapts to the unique capabilities and constraints of each platform. This principle is put into practice through the rigorous use of a **conditional import/export pattern**. This technique ensures that platform-specific logic—from leveraging browser-native APIs like Google Sign-In on the web, to providing user-friendly fallbacks like image downloads on desktop, to preventing web-only libraries from ever breaking a native mobile build—is cleanly isolated. This philosophy of deliberate adaptation ensures that the application is not merely cross-platform in theory, but robust, performant, and truly native-feeling in practice.

### Automated Testing as a Cornerstone of Quality and Maintainability
Automated testing is not an ancillary activity in the Haikuverse project; it is a foundational pillar of the development process and a non-negotiable aspect of professional software engineering. This commitment ensures high code quality, facilitates ongoing maintenance, and guarantees predictable application behavior. A comprehensive, multi-layered testing strategy, guided by a proactive, often **Test-Driven Development (TDD)** mindset, provides extensive test coverage —**currently exceeding 725 individual tests across models, providers, screens, and custom widgets**— enabling rapid iteration and feature development with unwavering confidence. This dedication was proven when a major dependency upgrade caused a catastrophic `build_runner` failure; instead of disabling tests, the issue was resolved by creating manual "nice" mocks to bypass the generator bug, restoring the full test suite.

The project's philosophy is rooted in an unyielding commitment to testable code design, primarily through **Dependency Injection (DI)**. This was pivotal for creating isolated tests for complex integrations, from AI services (simulating `http.Client` responses) and platform-specific functionalities (`image_picker_platform_interface`) to targeted **smoke tests** for platform-aware UI in the `SharingTab`. By injecting mock controllers and platform override flags, tests can verify UI logic (like a button's text changing) without touching untestable native APIs. Key techniques such as **strategic mocking** with `mockito`, realistic authentication simulation, **"golden" file** testing for custom painters, and careful **test isolation** (using `setUp`/`tearDown` and fixing test-only layout overflows) ensure thorough validation and prevent test bleed-over. This disciplined regimen serves as a critical **defense against regressions** and reflects a deep commitment to **engineering excellence.** Future enhancements will focus on implementing end-to-end `integration_test`s and dedicated testing for backend Cloud Function logic. Test execution is managed via standard `flutter test` and mock regeneration with `dart run build_runner build --delete-conflicting-outputs`.
...
### The Learning Loop: Prompt Transparency as Community Fuel
Haikuverse treats the "prompt" not as a disposable input, but as the "source code" of the art. The **Prompt Recovery** feature was introduced to operationalize the philosophy of "Open Source Creativity." By allowing any user to recover the original intent behind a published star, we transform the application from a passive gallery into an active learning laboratory. This transparency is essential for fostering a high-skill community (such as the **subreddit r/HaikuverseRealms**), where users don't just admire outcomes but actively deconstruct and share the semantic engineering techniques that created them.

### High-Fidelity Visuals: The "Clean Outline" Engine
The Haikuverse visual experience is powered by **Imagen 4**, selected for its superior semantic understanding. However, raw generative AI often drifts into "AI Slop"—a muddy, semi-photorealistic style lacking artistic intent. To combat this, the application implements a strict "Visual Engine" architecture centered on two pillars: **"Dynamic Lighting"** and **"Clean Outlines."**

This architectural choice forces the model to render decisive, vector-based edges rather than ambiguous textures, ensuring that a "Cyberpunk Mech" and a "Delicate Flower" share the same premium, illustrative DNA. To maximize semantic adherence, we employ a **"Direct Feed"** prompting strategy. We strip away conversational persona instructions and architecturally place the Haiku text before the User Input. This forces the model to treat the poem's atmosphere as the primary constraint, ensuring the generated image is an emotional reflection of the text rather than just a literal depiction of a noun.

For Constellation Fables, where users cannot prompt the visual generation directly, Imagen 4 serves a distinct architectural role: **"Visual Conductor."** It enforces a unified, ethereal aesthetic (incorporating motion and energy trails) across the semantic Knowledge Graph. This ensures that as users traverse from constellation to constellation, the visual language remains cohesive and immersive, regardless of the disparate themes being explored.

To ensure reliability, this high-fidelity engine employs a **"Graceful Degradation"** pattern. If the Imagen 4 model fails (due to aggressive rate limiting or quotas), the system automatically and silently falls back to the standard **Imagen 3** model, ensuring that the user's creative flow is never interrupted by infrastructure constraints.

### Ethical Monetization and Inclusive Access
The application’s business model is a direct extension of its "Designed for Humans" philosophy, engineered to sustain the creative ecosystem, not exploit it. Inclusive access is built-in through a **tiered subscription model available via the Google Play Store** that provides an affordable entry point for casual poets while offering greater resources for dedicated creators. This is complemented by flexible, non-expiring "pay-as-you-go" **Quota Boosts**, allowing users to manage their usage without pressure.
...
Once inside, the core experience continues this philosophy, prioritizing intuitive engagement from the very first moment. This is exemplified by the "Welcome, Poet!" message, an instructional block that clarifies the "Master Prompt" concept, gently guiding the user to prevent confusion. This principle of clear, low-friction interaction extends to the straightforward `HomeScreen prompt`, the unambiguous `FavoriteButton`, and the at-a-glance `StarProgressTracker` checklist. It's also present in the tactile, tap-to-navigate gestures of the `MiniConstellationGraphView` and is further refined by small but impactful UX features, like reversing the favorites list to show the newest creations first, dynamically changing a modal's text from 'Overwrite' to 'Save' to match the user's context, a dedicated button to clear the prompt text field, and the `SmartScrubberModal` search tool for the favorites list that respects a power-user's time. Every element is designed to remove minor annoyances and feel like a natural extension of the user's creative intent.
...
### A Production Crisis Averted: The Value of Key Redundancy
The 1.0.13+40 release was blocked by a high-stress production crisis that validated the project's data redundancy strategy. The `flutter build appbundle` command, which had worked for 39 prior builds, suddenly failed during the `:app:signReleaseBundle` task with a `java.lang.NullPointerException`. This was traced to a misconfigured `android/key.properties` file that could not find or access the signing key. In the initial troubleshooting, a *new* upload key was mistakenly generated. This new key was, of course, rejected by Google Play with a "signed with the wrong key" error. The crisis was ultimately averted when the **original, correct** production key (`haiku-bot-advanced-release.jks`) and its password file were retrieved from the secure `haiku_bot_secrets` backup folder. This allowed the `android/key.properties` file to be correctly configured, enabling an immediate and successful build that Google Play accepted.

More importantly, a key architectural decision that provides peace of mind for rapid recovery of the Google Play upload key was the project's enrollment in **"Signing by Google Play"**. This enrollment means that a lost key in the developer's IDE is still a *replaceable* "upload key," not the *irreplaceable* "app signing key" managed by Google. This provides two distinct recovery paths: either submit the formal reset request and accept the 48-hour security hold, or use a local backup.

### Validating Data Integrity: A Lesson in Manual Inspection
A critical lesson was learned not through a build failure, but through manual, production-data inspection during the closed Alpha test. The original architecture for monetization relied on a scheduled daily batch job (`resetMonthlyCounters`) to reset user quotas. While this passed all local and automated tests, **manual inspection of the Firestore database** after a real billing cycle revealed a critical flaw: the batch logic was incorrect and failed to reset quotas, which would have led to a catastrophic failure that would have allowed unsubscribers to continue using the app with the same monthly quota reset as subscribers.

This discovery proved that assumptions about data, especially time-based billing logic, must be validated against real-world state. The flawed batch job was completely removed and replaced by the event-driven `handlePlayStoreNotification` listener. This new architecture, which reacts instantly to Google Play's RTDN events, is inherently more resilient and correct—a fix driven entirely by the "lessons learned" from trusting, but verifying, the production data itself.

### Validating the Full Subscription Lifecycle: Always Read the Latest API!
A critical production lesson learned was that the full subscription lifecycle (purchasing, renewing, lapsing, and re-subscribing) is deceptively complex, revealing interconnected bugs in both the backend and the client. The event-driven `handlePlayStoreNotification` listener, which replaced an old batch job, introduced subtle production bugs. Analysis of GCP logs was critical in discovering that the listener's logic was inverted (e.g., treating `Type 2 RENEWED` as "Canceled") and was missing the official `Type 13 EXPIRED` event. This bug would have silently granted permanent free subscriptions. This lesson emphasized that real-time notification systems require meticulous validation of every event code against the latest **official** documentation. Furthermore, a race condition where the `Type 4 (PURCHASED)` event arrived before `validateAndroidPurchase` could save the token, had to be resolved by making `validateAndroidPurchase` the sole authority for new purchases rather than `handlePlayStoreNotification`.

However, fixing the `Type 13 EXPIRED` event immediately exposed a second, critical bug in the client's state management. When a lapsed user's subscription was correctly set to false, they were booted to the /subscription screen. Upon re-subscribing, they were let in through the "side door" (the QuotaProvider's `isSubscriptionActive` flag) which bypassed the initial `authStateChanges` sign-in event. This resulted in a functional but completely empty app, as the data-loading functions (`_subscribeToFavorites`, etc.) were never called. The fix required adding a new listener (a "side door bell") directly within the AppShell's build method, using a Consumer to watch the `QuotaProvider`. This new logic now detects the false to true subscription change and manually re-triggers all data-loading functions, ensuring the app is fully populated regardless of which "door" the user enters through.

### Navigating Cross-Platform Tooling and Dependency Hell
A significant maintenance effort involved upgrading core dependencies, which triggered a cascade of platform-specific issues that were invisible in their respective isolated environments, highlighting the critical need for a robust cross-platform architecture. The process began when a `flutter pub upgrade` introduced a new version of the `analyzer` package, exposing a bug in `mockito`'s code generator. This caused `build_runner` to crash and block all automated tests for certain files.

The initial fix—creating manual mocks to bypass the faulty generator—led to a second, more insidious problem. The introduction of the modern `package:web` for a sharing feature regression caused the entire native Android build to fail with thousands of errors, as web-only libraries were being compiled for a native target. The final, robust solution required encapsulating all web-specific logic behind a **conditional import/export pattern**, ensuring the native compiler would never attempt to read the web-only code.
