...
# Run specific suites
npm run test:api   # Backend Routes only
npm run test:lib   # Service Logic only

# Generate Coverage Report (The "Audit")
...
- **API Integration Tests** (`__tests__/api/`):
  - **Focus:** The critical "Controllers" managing money, data, and security.
  - **Coverage:** Achieved 100% Regression Coverage across all 11 API routes, including complex Stripe/Gelato webhooks.
  - **Strategy:** Uses robust Mock Factories to simulate external services (Stripe, Firebase, Sharp) and Isolation to verify disaster recovery scenarios (e.g., database crashes, missing environment variables) without incurring cloud costs.

- **Unit Tests** (`__tests__/lib/`):
  - **Focus:** Pure business logic and data transformation layers.
  - **Goal:** Verify that the `firebaseService` correctly maps Firestore types (Timestamps) to serializable JSON and that `products.ts` accurately calculates dynamic pricing for Foil vs. Standard orders.
...
### 5.12 The Invisible Safety Net: Automated Integrity

In the Fancyland ecosystem, we view software reliability as a form of empathy. A crashed payment webhook is not merely a server error; it is a betrayal of a user who trusted us with their financial data. A failed "Order Shipped" email is a stolen moment of anticipation.

To honor this trust, we rejected the "move fast and break things" orthodoxy in favor of a **"Zero-Cost Regression"** strategy. We engineered a comprehensive suite of over 150 automated tests that acts as a robotic auditor for the entire backend infrastructure. Before a single line of code is deployed to production, this suite simulates catastrophic failures—database outages, missing encryption keys, and network crashes—to ensure the system fails safely and gracefully. This infrastructure allows us to innovate rapidly without fear, knowing that the logic governing our users' money and privacy is mathematically verified to be bulletproof.
